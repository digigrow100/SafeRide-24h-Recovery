      ...
      <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-12">
        <div className="relative order-2 lg:order-1 lg:col-span-5">
          <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-secondaryBg shadow-xl">
            ...
            <div className="absolute left-4 top-4 rounded border border-primaryNeon/30 bg-panel/90 px-3 py-1.5 backdrop-blur">
              <span className="font-mono text-[10px] uppercase tracking-widest text-primaryNeon">
        ...
        <div className="order-1 flex flex-col gap-6 lg:order-2 lg:col-span-7">
          ...
          <div className="grid grid-cols-1 gap-4 pt-1 sm:grid-cols-2">
            {PROTOCOL_ITEMS.map((item) => (
              <div
                key={item.title}
                className="flex flex-col gap-2 rounded border border-secondaryNeon/20 bg-panel p-4 transition-colors"
              >
                <div className={`flex items-center gap-2 ${item.accent}`}>
                  <span className="material-symbols-outlined text-[20px]">{item.icon}</span>
                  <span className="font-mono text-sm font-bold uppercase">{item.title}</span>
                </div>
